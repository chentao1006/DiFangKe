...
interface FootprintDao {
        ...
        @Query(
                "SELECT * FROM footprints WHERE startTime < :end AND endTime > :start AND statusValue != 'ignored' ORDER BY startTime ASC"
        )
        ...
        @Update suspend fun update(footprint: FootprintEntity)

        @Delete suspend fun delete(footprint: FootprintEntity)

        @Query("DELETE FROM footprints WHERE footprintID = :id") suspend fun deleteById(id: String)

        @Query("SELECT COUNT(*) FROM footprints WHERE statusValue != 'ignored'")
        suspend fun count(): Int

        @Query(
                "SELECT * FROM footprints WHERE startTime >= :start AND startTime < :end AND statusValue = 'ignored' ORDER BY startTime ASC"
        )
    ...
    """
        ...
        @Query(
                "SELECT DISTINCT date(startTime/1000, 'unixepoch', 'localtime') FROM footprints WHERE statusValue != 'ignored' ORDER BY startTime ASC"
        )
        fun observeAvailableDates(): Flow<List<String>>

        @Query(
                "SELECT * FROM footprints WHERE placeID = :placeID AND startTime < :currentTime AND statusValue != 'ignored' ORDER BY startTime DESC LIMIT 1"
        )
        suspend fun getLastVisitToPlace(placeID: String, currentTime: Date): FootprintEntity?

        @Query(
                "SELECT * FROM footprints WHERE locationHash = :hash AND startTime < :currentTime AND statusValue != 'ignored' ORDER BY startTime DESC LIMIT 1"
        )
        suspend fun getLastVisitToHash(hash: String, currentTime: Date): FootprintEntity?

        @Query("DELETE FROM footprints") suspend fun deleteAll()

        @Query("DELETE FROM footprints WHERE startTime >= :start AND startTime < :end")
        suspend fun deleteBetween(start: Date, end: Date)

        @Query(
                "DELETE FROM footprints WHERE startTime >= :start AND startTime < :end AND statusValue = 'candidate' AND isTitleEditedByHand = 0"
        )
